...
def get_monthly_working_hours(employee=None, year=None, month=None):
	...
	filters = {"attendance_date": [">=", start_date], "attendance_date_end": ["<", end_date], "docstatus": 1}
    ...
    """.format(employee_filter="AND a.employee = %(employee)s" if employee else "")

	params = {"start_date": start_date, "end_date": end_date, "employee": employee}
	...
	for r in result:
		...
		total_hours += hours
		print(
			f"{r.employee:<15} {r.employee_name:<12} {r.present_days:>6} {r.absent_days:>6} {r.leave_days:>6} {r.half_days:>6} {hours:>10.2f}h"
		)
...
def get_employee_daily_detail(employee, year=None, month=None):
	...
	result = frappe.db.sql(
		sql, {"employee": employee, "start_date": start_date, "end_date": end_date}, as_dict=True
	)
	...
	for r in result:
		...
		print(
			f"{r.attendance_date!s:<12} {r.status:<10} {in_time:>10} {out_time:>10} {hours:>8.2f}h {late:>6} {early:>6}"
		)
...
def export_monthly_attendance(year=None, month=None, employee=None, output_dir=None):
	...
	result = frappe.db.sql(
		sql, {"start_date": start_date, "end_date": end_date, "employee": employee}, as_dict=True
	)
...
def export_monthly_summary(year=None, month=None, output_dir=None):
	...
	result = frappe.db.sql(sql, {"start_date": start_date, "end_date": end_date}, as_dict=True)
