...
def execute():
	...
	return {"created_assignments": created, "skipped_assignments": skipped, "updated_checkins": updated}
...
def full_migration():
	...
	return {"cancelled_assignments": cancelled, "created_assignments": created, "updated_checkins": updated}
...
def cancel_all_shift_assignments():
	...
	assignments = frappe.get_all(
		"Shift Assignment",
		filters={"docstatus": 1, "shift_type": ["!=", SHIFT_NAME]},
	...
	unified_assignments = frappe.get_all(
		"Shift Assignment", filters={"docstatus": 1, "shift_type": SHIFT_NAME}, pluck="name"
	)
...
def update_all_checkins_to_unified_shift(shift):
	...
	# 统计需要更新的记录
	# 1. offshift 记录
	offshift_count = frappe.db.count("Employee Checkin", {"offshift": 1, "attendance": ["is", "not set"]})
    ...
    """,
		{"shift_name": SHIFT_NAME, "start_time": start_time, "end_time": end_time},
	)
...
def create_shift_assignments(shift):
	...
	for emp in employees:
		...
		existing = frappe.db.exists(
			"Shift Assignment",
			{"employee": emp.name, "shift_type": SHIFT_NAME, "docstatus": 1, "status": "Active"},
		)
		...
		try:
			...
			print(f"   ✗ {emp.name} - {emp.employee_name}: {e!s}")
...
def rollback():
	...
	print("[步骤2] 取消班次分配...")
	assignments = frappe.get_all(
		"Shift Assignment", filters={"shift_type": SHIFT_NAME, "docstatus": 1}, pluck="name"
	)
