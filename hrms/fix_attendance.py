...
def fix_employee_jan5_attendance(employee="50", date="2026-01-05"):
	...
	attendances = frappe.get_all(
		"Attendance", filters={"employee": employee, "attendance_date": date}, pluck="name"
	)
	...
	checkins = frappe.db.sql(
        ...
        ORDER BY time
    """,
		(employee, f"{date} 00:00:00", "2026-01-06 04:00:00"),
		as_dict=True,
	)

	for c in checkins:
		print(
			f"  {c.name}: {c.time} {c.log_type} | shift_actual_end={c.shift_actual_end} | att={c.attendance}"
		)
	...
	for att in new_attendance:
		print(
			f"  {att.name}: {att.attendance_date} | in={att.in_time} | out={att.out_time} | hours={att.working_hours}"
		)
