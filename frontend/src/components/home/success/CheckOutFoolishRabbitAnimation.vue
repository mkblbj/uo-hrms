<template>
	<!-- Source: https://uiverse.io/whoisyourdeadie/foolish-rabbit-13 (MIT License). -->
	<div class="matrix-container">
		<div v-for="patternIndex in 5" :key="`matrix-pattern-${patternIndex}`" class="matrix-pattern">
			<div
				v-for="columnIndex in 40"
				:key="`matrix-column-${patternIndex}-${columnIndex}`"
				class="matrix-column"
			></div>
		</div>
	</div>
...
@keyframes rainbowShift {
	0% {
		color: linear-gradient(0deg, red, orange, yellow);
	}
	50% {
		color: linear-gradient(0deg, green, blue, indigo);
	}
	100% {
		color: linear-gradient(0deg, violet, red, orange);
	}
}
