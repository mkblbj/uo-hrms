...
| 结束时间 | 23:59 |
| 启用自动考勤 | ✅ |
| 签到签退判定方式 | Alternating entries as IN and OUT during the same shift |
| 工时计算方式 | First Check-in and Last Check-out |
| 允许提前签到(分钟) | 1440 (24小时) |
| 允许延后签退(分钟) | 1440 (24小时) |
| **启用时间取整** | ✅ |
| **取整精度** | 15分钟 |
...
> 注：开始/结束时间设为全天是为了兼容弹性工作时间，实际工时由签到签退决定。
...
### 阶段一：字段扩展
- [ ] 修改 `shift_type.json` 添加新字段
- [ ] 运行 `bench migrate`
- [ ] 验证新字段在界面显示

### 阶段二：计算逻辑
- [ ] 添加 `round_time_to_precision` 函数
- [ ] 添加 `calculate_lunch_overlap_hours` 函数
- [ ] 修改 `get_attendance` 或 `calculate_working_hours`
- [ ] 单元测试验证计算正确性
...
### 阶段五：考勤重算
- [ ] 设置 process_attendance_after 日期
- [ ] 运行 process_auto_attendance
- [ ] 抽检考勤记录正确性
